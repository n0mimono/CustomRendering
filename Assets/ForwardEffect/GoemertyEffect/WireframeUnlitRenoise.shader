    ...
    Pass {
      ...
      void geo(triangle v2f v[3], inout TriangleStream<v2f> TriStream) {
        ...
        for (int i = 0; i < 3; i++) {
          ...
          float4 wpos = mul(unity_ObjectToWorld, v[i].vertex);
          v[i].vertex.xyz += normal * max(0, t - 0.5 * wpos.y) * 10 * (2 * t + snoise(2 * wpos.xyz));
          //v[i].vertex.xyz = twist(v[i].vertex.xyz, 5 * t + snoise(wpos).x);
